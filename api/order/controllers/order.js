...
/**
 * Given a dolllar amount, return the amount in cents
 * @param {number} number  
 */
...
module.exports = {
    ...
    async find(ctx) {
       ...
       /**
        * Returns one order, as long as it belong to the user
        * @param {any} ctx  
        */
     async findOne(ctx) {
        const { id } = ctx.params
     ...
     /**
      * Craete an order and sets up Stripe checkout session for the frontend
      * @param {any} ctx 
      */
     async create(ctx){
    ...
    async confirm(ctx) {
        ...
        if(session.payment_status === 'paid'){
            ...
            ctx.throw(400, "The payment wasn't succesfull, Please call support")
        }
    }
